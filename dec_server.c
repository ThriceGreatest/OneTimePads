...
void decrypt(char ptxt[], char key[], char cipher[], int size){
	...
	for(i = 0; i < size; i++){
		...
		//Modulus was causing problems... somehow this formula seems to work.
		sum = intPTXT[i] - intKEY[i];
		if(sum < 0){
			sum = 26 + sum;
		}
...
int main(int argc, char *argv[])
{
	...
	//	printf("Size = %d\n", size_sent);
		...
		case 0:
			...
			decrypt(plaintext, key, cipher, size);
